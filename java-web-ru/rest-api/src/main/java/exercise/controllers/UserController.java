...
public class UserController implements CrudHandler {
    ...
    public void getOne(Context ctx, String id) {
        ...
        // BEGIN
        User user = new QUser().id.equalTo(Long.parseLong(id)).findOne();
    ...
    public void create(Context ctx) {
        ...
        User user = ctx.bodyValidator(User.class)
                ...
                .check(it -> EmailValidator.getInstance().isValid(it.getEmail()), "Email should be valid")
                .check(it -> it.getPassword().length() >= 4, "Пароль должен быть длиннее 3 символов")
    ...
    public void delete(Context ctx, String id) {
        // BEGIN
        new QUser()
                .id.equalTo(Long.parseLong(id)).delete();
        // END
    };
}
